    ...
    <AuthenticatedLayout>
        <div v-if="showForm" class="mb-8 p-6 border-2 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div class="mt-4 flex gap-x-2">
                <button @click="submit" class="bg-black text-white px-6 py-2 font-bold uppercase">Save Product</button>
        ...
        <DataTable :resource="products" :columns="columns">
            ...
            <template #actions="{ row }">
                <div class="flex flex-row gap-x-[15px] justify-end uppercase text-xs font-black">
                    ...
                    <button @click="$inertia.delete(route('products.destroy', row.id))" class="underline text-red-500">Delete</button>
                </div>
            </template>
        </DataTable>
    </AuthenticatedLayout>
