...
const deleteCategory = (id) => {
    if (confirm('APAKAH ANDA YAKIN INGIN MENGHAPUS KATEGORI INI? (DATA AKAN DIARSIPKAN)')) {
    ...
    <AuthenticatedLayout>
        <div class="p-8">
            ...
            <div v-if="showInlineForm" class="mb-8 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                <div class="bg-gray-50 border-b border-gray-200 px-6 py-4 flex justify-between items-center text-gray-700">
                    <h2 class="text-sm font-black uppercase tracking-widest">➕ Tambah Kategori Baru</h2>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-1">
                            ...
                            <input v-model="form.name" type="text" placeholder="CONTOH: MAKANAN, MINUMAN, ELEKTRONIK..." 
                                class="w-full border border-gray-300 rounded-lg p-3 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all uppercase placeholder:font-normal placeholder:text-gray-300"
                                @keyup.enter="submit" />
                            <span v-if="form.errors.name" class="text-[10px] font-bold text-red-500 uppercase mt-1 tracking-tighter">{{ form.errors.name }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex gap-3 border-t border-gray-100 pt-6">
                        ...
                        <button @click="showInlineForm = false" 
                            class="bg-white border border-gray-300 text-gray-500 px-8 py-2.5 rounded-lg text-xs font-bold uppercase hover:bg-gray-50 transition-all tracking-widest">
                            Batal
                        </button>
                    </div>
                </div>
            ...
            <div v-if="showModalForm" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                <div class="bg-white w-full max-w-md rounded-xl shadow-2xl border border-gray-200 overflow-hidden animate-in zoom-in-95 duration-200">
                    <div class="bg-gray-50 px-6 py-4 border-b flex justify-between items-center">
                        <h2 class="text-sm font-black text-gray-700 uppercase tracking-widest">✏️ Edit Kategori</h2>
                        <button @click="showModalForm = false" class="text-gray-400 hover:text-red-500">✕</button>
                    </div>
                    <div class="p-8 space-y-5">
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-black text-blue-600 uppercase tracking-widest">Nama Kategori</label>
                            <input v-model="form.name" type="text" 
                                class="w-full border-2 border-blue-100 rounded-lg p-3 text-lg font-black text-blue-700 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all uppercase" 
                                @keyup.enter="submit" />
                            <span v-if="form.errors.name" class="text-[10px] font-bold text-red-500 uppercase mt-1">{{ form.errors.name }}</span>
                        </div>
                        <div class="flex gap-3 pt-4">
                            <button @click="submit" :disabled="form.processing" 
                                class="flex-1 bg-blue-600 text-white py-3.5 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all active:scale-95">
                                Update Data
                            </button>
                            <button @click="showModalForm = false" 
                                class="px-6 py-3.5 border border-gray-300 rounded-lg text-xs font-bold uppercase text-gray-500 hover:bg-gray-50 tracking-widest">
                                Tutup
                            </button>
                        </div>
                    </div>
