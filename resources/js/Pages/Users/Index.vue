...
const columns = [
    ...
    { label: 'Terdaftar Pada', key: 'created_at', sortable: true }
];
...
const destroy = (id) => {
    if (confirm('APAKAH ANDA YAKIN INGIN MENGHAPUS DATA INI?')) {
        // Menggunakan router.delete untuk konsistensi dengan controller destroy
        router.delete(route('users.destroy', id), {
            preserveScroll: true,
            onSuccess: () => {
                // Notifikasi berhasil ditangani oleh flash message jika ada
            }
        });
    }
};
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input v-model="form.email" type="email" placeholder="contoh@mail.com" :readonly="isUpdate"
                                :class="[
                                    isUpdate ? 'text-gray-500 bg-gray-50' : '',
                                    form.errors.email ? 'border-red-500' : ''
                                ]"
                                class="w-full border border-gray-300 rounded-lg p-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all placeholder:text-gray-300" />
                    ...
                    <div class="mt-8 flex gap-3">
                        <button @click="submit" :disabled="form.processing" 
                            ...
                            {{ form.processing ? 'Memproses...' : (form.id ? 'Simpan Perubahan' : 'Daftarkan User') }}
                        </button>
