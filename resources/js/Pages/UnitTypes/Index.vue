        ...
        <div class="p-8">
            ...
            <div v-if="showInlineForm" class="mb-8 bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                <div class="bg-gray-50 border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                    <h2 class="text-sm font-bold text-gray-700 uppercase tracking-wider">➕ Tambah Satuan Baru</h2>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Nama Satuan</label>
                            <input v-model="form.name" type="text" placeholder="CONTOH: PCS, KG, BOX..." 
                                ...
                                @keyup.enter="submit" />
                            <span v-if="form.errors.name" class="text-[10px] font-bold text-red-500 uppercase">{{ form.errors.name }}</span>
                        </div>
                    </div>

                    <div class="mt-8 flex gap-3 border-t border-gray-100 pt-6">
                        <button @click="submit" :disabled="form.processing" 
                            class="bg-blue-600 text-white px-8 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-sm active:scale-95">
                            Simpan Satuan
                        </button>
            ...
            <div v-if="showModalForm" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
                <div class="bg-white w-full max-w-md rounded-xl shadow-2xl border border-gray-200 overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div class="p-8 space-y-5">
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Nama Satuan</label>
            ...
            >
                ...
                <template #actions="{ row }">
                    <div class="flex flex-row gap-4 justify-end">
                        ...
                        <button @click="deleteUnit(row.id)" class="text-gray-300 hover:text-red-600 transition-colors" title="Hapus">❌</button>
                    </div>
                </template>
            </DataTable>
        </div>
