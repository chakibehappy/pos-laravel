...
watch(search, debounce((value) => {
    router.get(route('topup-trans-types.index'), { search: value }, { preserveState: true, replace: true });
}, 500));
...
const resetSingleEntry = () => {
    singleEntry.value = { name: '', type: 'digital' };
};
            ...
            <div v-if="showForm" :class="isEditMode ? 'fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm' : 'mb-8 p-6 bg-white rounded-xl border border-gray-200 shadow-md relative animate-in fade-in zoom-in duration-200'">
                ...
                <div :class="isEditMode ? 'bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-100 relative' : 'w-full'">
                    ...
                    <div v-if="isEditMode" class="p-6 border-b border-gray-100 bg-gray-50 rounded-t-2xl">
                        <h2 class="text-xl font-black text-gray-800 uppercase tracking-tighter">Edit Tipe Transaksi</h2>
                    </div>
                    ...
                    <div :class="isEditMode ? 'p-8 grid grid-cols-1 md:grid-cols-2 gap-6' : 'grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200'">
                        ...
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Kategori (Type)</label>
                            <select v-model="singleEntry.type" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none uppercase">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <tr v-for="(tr, idx) in form.items" :key="idx" class="hover:bg-blue-50/30 transition-colors">
                            ...
                            <td class="p-3 uppercase">
                                <span :class="tr.type === 'digital' ? 'text-blue-600' : 'text-purple-600'" class="font-black italic">{{ tr.type }}</span>
                            </td>
